...
class PriceShockers:
    ...
    def __init__(self):
        ...
        self.full_url = f"{self.BASE_URL}/price_shockers"
    ...
    def _upload_data(self, data: dict):
        ...
        blob_file_path = f"test_price_shockers/price_shockers.txt"
        data_str = json.dumps(data)
        try:
            existing_files = self.supabase.storage.from_("pdf_chunks").list("test_price_shockers")
            file_exists = any(file['name'] == f"price_shockers.txt" for file in existing_files)
            if file_exists:
